...
- [x] 3.13 `validate.test.ts:206-216` — retarget off `/not generated with --icm/`; coordinate with 4.2's new message. This is the fourth premise-dies test and is **not** in the spec's §5 table. **DISCHARGED BY 4.5** — verified at unit 6.0: `grep "not generated with" packages/create-agent-harness/__tests__/validate.test.ts` returns nothing (the assertion is retargeted off the deleted string), and 4.5's own record states "This also discharges 3.13, which was blocked on this message rewrite." Left open here originally as a deliberate block; now closed with evidence rather than re-done.
...
- [x] 4.3 Define the flip version as a named constant (`FLIP_VERSION`) set to the release cut in 6.6, and read it against `manifest.generatorVersion` (already recorded at scaffold time). Document that an absent/unparseable version takes the **carve-out** path — fail toward silence, not toward a false WARN. ❌ **NOT IMPLEMENTED — FALSIFIED ON TWO INDEPENDENT FACTS; WITHDRAWN AND SPEC AMENDED 2026-09-16.** (i) The field does not exist: `HarnessManifest` records `generator` (`manifest.ts:49`), so `manifest.generatorVersion` is `undefined` on every scaffold ever produced. (ii) Even with the right name, the value discriminates nothing: every code path stamps the hard-coded literal `'0.1.0'` (`index.ts:1313`, `analyze-repo.ts:426`), and no release/build/script step threads the real package version (`0.4.16`) in — so a pre-removal and a post-flip harness both record `generator: "0.1.0"` and no comparison against any `FLIP_VERSION` can separate them. Worse, since `0.1.0` is *always* below the flip, the prescribed predicate would route **every** capable-tree-less harness to the silent carve-out, making the `WARN` arm unreachable. Corroborating: the existing generator-skew machinery (`diag.ts`) reads the same field and reports `minor-diff` on a real harness — it too cannot tell pre- from post-removal. Substituted: one honest `WARN` covering both readings. See `02-proofs/02-task-04-proofs.md` → *"Task 4.3 is falsified"*. **Resolution (Chris approved, 2026-09-16):** the false `generatorVersion` clause is now amended out of the spec rather than left standing — §3.4:178 carries a dated amendment block and SC4:199 now reads "withdrawn as unimplementable", with the two-case count made explicit. The originating round-1 claim in `02-questions-1-icm-default-on.md` is annotated in place too. Every prescribed sub-task in this phase is now either implemented or withdrawn-with-recorded-evidence; none is silently skipped.
...
spec's §3.4 table body and prose already describe the landed behaviour; the
`generatorVersion` clause at §3.4:178 and SC4:199 contradicted it, and **both are
now amended (2026-09-16, Chris-approved)** — §3.4 carries a dated amendment block
and SC4 names the carve-out as withdrawn with the two-case count. The round-1
claim in `02-questions-1-icm-default-on.md` is annotated in place as
believed-then-known-false, so a later grep for `generatorVersion` lands on the
correction rather than on three unlinked assertions.
...
### [x] 5.0 `upgrade` regression guard and the residual question count
...
- [x] 5.1 Capture the pre-removal baseline: scaffold a flagless `vertical:coding` harness from the **pre-flip** behaviour, strip the timestamp, and commit it under `__tests__/fixtures/icm-preremoval/` with its manifest recording **no** ICM paths. (Precedent: spec 01's `arc-pre-removal-tools.json` — a committed baseline is what makes the claim auditable.) **Done:** 19 files recorded, 0 ICM paths, template `vertical:coding`, generator `0.0.0`, `generated_at` masked; produced through the internal `scaffold({ icm: false })` override (pre-task-2.4 CLI behaviour). Manifest only, not a 19-file tree — `upgradeCmd` never reads harness contents to decide ICM-ness.
- [x] 5.2 Add the SC5 regression test: run `upgradeCmd` against that fixture and assert **zero** ICM files are reported as added *or* removed. State in the test comment that this holds only because `icmEnabled(manifest)` reads the harness, not the CLI. **Done:** `upgrade.test.ts:119-209`, three tests. Baseline computed *independently* of the code path under test (`walkTemplate(…, { icm: false })`), plus a "guards the guard" case asserting the fixture is ICM-free while its template is capable. **Mutation-falsified:** merging `icmEnabled` → `resolveIcmDefault` reddens **2 of 3**, including the meta-test written to catch it.
- [x] 5.3 Comment `upgrade-cmd.ts:44` as load-bearing: a pre-removal harness has no ICM paths in its manifest, so a capability-derived re-render would add 10 files to a harness the user never opted into. Do **not** change its behaviour. **Done:** comment at `:43-58` names the anti-pattern explicitly, plus the "Two resolvers" rationale. Behaviour unchanged — `grep -n "icmEnabled" src/upgrade-cmd.ts` still returns only `:64` (definition) and `:108` (call). The 10-file figure is now **measured** in test 3, not asserted.
- [x] 5.4 **Measure before pinning** (spec §4.1). Run the capable scaffold flagless and count (i) distinct *unanswered* catalog question ids and (ii) raw `scanResiduals` markers. Expected shape: 8 tokens today, 6 questions after the fix — but **record what you measure**, not what the spec predicts. The criterion is the semantics; the number is an output of implementation. **Measured:** raw **8** → questions **6**, delta **2** = the two `{{?COND}}` openers (`?SUBAGENT_HANDOFF`, `?FIX_LOOP`); both closers excluded as structure. `minimal` flagless: 0 (off by default, Q1 as designed); under the override: 3. Matches §4.1's anticipated shape.
- [x] 5.5 Give `scanResiduals` an optional known-question-id set (the catalog's `icm.questions`) and count only markers whose id is a **real question**; report `{{?COND}}` / `{{/COND}}` conditional markers through the structural report instead of the question count. Preserve its split from the walker's `unresolved[]` (lowercase Mustache) — the doc comment at `:315-319` explains why folding them would double-report. **Done:** `onboarding.ts:338-369`. **One deviation, recorded:** the conditionals are *collapsed to their question id*, not routed to a "structural report" — `{{?FIX_LOOP}}` **is** the unanswered question `FIX_LOOP`, so excluding it would undercount. The closers are dropped as structure (`:351`). The walker split is preserved; `undeclared` ids are surfaced separately at `validate.ts:399` rather than folded in. See `02-proofs/02-task-05-proofs.md`.
- [x] 5.6 Update the callers in `scaffold()`'s per-file pass to supply the id set; keep the batch entry point working. **Done:** `index.ts:769-775` reads the id set from the same catalog entry `resolveIcmDefault` consults; per-file callers at `:950` and `:963` supply it. `validate.ts:405` uses the same source. Batch entry point `onboardFiles` forwards it as an optional param. ⚠️ **Finding F2:** `onboardFiles` has **no caller in the repository** — pre-existing (`git grep` at `f59fbe7` shows the same lone definition), not introduced here. It works, but carries no test and no caller; recorded so a later grep does not read it as a miss.
- [x] 5.7 Update the residual message string to say "question(s)" and add tests for `vertical:coding` and `minimal` (different question counts), plus an assertion that a conditional marker is **not** counted as a question. **Done:** strings at `index.ts:1349,1355` and `validate.ts:412`; tests at `onboarding.test.ts:379,398,405` (conditional-as-its-question, undeclared excluded, per-template 6 vs 3) and `validate.test.ts:252-253` (`toMatch(/2 ICM question\(s\)/)` + `not.toMatch(/placeholder/)`).
- [x] 5.8 Record the measured value in the task proof and in the changelog-facing note. If it differs from what §4.1 anticipates, say so plainly rather than adjusting the measurement to fit — the spec now states semantics and defers the number, so a mismatch is a finding, not a failure. **Done:** `02-proofs/02-task-05-proofs.md`, plus a lift-ready changelog note. §4.1's 8→6 shape is confirmed exactly. ⚠️ **A second undercount was found underneath it:** the old source (`render().unresolved`) reported **4**, missing `SUBAGENT_HANDOFF` and `FIX_LOOP` entirely (both appear *only* in conditional form). §4.1 anticipated the 8→6 collapse; it did not anticipate the 4→6 correction. Recorded at `validate.ts:361-369`. The release entry itself stays with 6.6/SC10 — this task supplies the measured number only.

> **Finding F1 (5.0, not fixed — flagged for the spec author):** a flagless `minimal` harness reports `SKIP icm-structure — template is not ICM-capable`. `minimal` **is** capable; it is `generate: false`, i.e. capable-and-suppressed-by-default. `resolveIcmDefault` folds both conjuncts to `false`, so the SKIP string asserts something false about the template. The states remain distinguishable (`entry.icm?.enabled` is readable there), so it is a wording fix inside §7's three-way branch — not new machinery. Not fixed here: SC4's contract names two arms, and §7 is the spec author's artifact to amend.
>
> **✅ CLOSED 2026-09-17 (spec author).** The predicted shape was exactly right — a wording fix, no new machinery. SC4 was amended to name three arms and §3.4's table grew the third row; `index.ts` now exports `isIcmCapable(entry)` beside `resolveIcmDefault(id)` so the extra conjunct is never re-encoded at a second site (§6.2 preserved). Tag/exit unchanged for every arm. Verified by a new `validate.test.ts` case and a live four-case probe, including proof that the message's `icm: true` advice actually produces `minimal`'s tree. See §4.1's amendment note.

### [x] 6.0 CI tour, supersession record, and release
...
- [x] 6.1 Re-run the tour's ICM pass and confirm `minimal` and `vertical:coding` both still pass (F1's acceptance test at the CI level). If `minimal` fails, the override was dropped somewhere — fix the code, not the assertion. → `ICM pass: 2/2 OK`; `minimal` still emits under the override.
- [x] 6.2 **Close the default-coverage hole.** Add a flagless capable-template assertion to a CI-visible check (the tour's main pass or `healthcheck.mjs`) so the new default is exercised. Without this, every CI assertion about ICM uses the explicit override and the *default* — the entire point of the change — is untested. → `icmDefaultCheck` + flagless pass over all 20 templates, `20/20 OK`. Oracle reads **catalog fields**, not `resolveIcmDefault` (which `scaffold()` itself calls) — otherwise the mutation it guards moves both sides together. Mutation (drop `generate !== false`) → `minimal: default false but an ICM tree was emitted`, `EXIT=1`.
- [x] 6.3 Rewrite the tour's ICM-pass rationale (`:81-87`, `:85`, `:192`, `:229`): it justifies the separate pass by "the flagless path is the byte-equality guarantee". Keep the separate pass (it prevents ambiguous file counts), retire the reason, and state the new one — the override-driven pass proves per-template emission while 6.2 proves the default. → all four sites; two also named the now-deleted `--icm` flag and were corrected past-tense.
- [x] 6.4 Write `docs/adrs/ADR-285-<slug>.md` with the `Supersedes ADR-279 §2; amends §3 rationale; §1 and §4 stand` header. Record: the capability predicate and why two conjuncts; the override's survival and why (`walkTemplate` has two callers); F1 explicitly (that a bare capability default would have made `minimal`'s tree unemittable); and the honest phrase **byte-equality retired, capability-preservation substituted**. → `ADR-285-icm-default-on-capability-derived-emission.md`. Two citation corrections made during verification (`useIcm` is `:768` not `:753`; ADR-282's false claims are *paragraphs* under two headings, not `§`-numbered sections). File-count figure corrected to the measured **21 → 31, delta 10**, with the 19-vs-21 fixture/manifest gap explained.
- [x] 6.5 Add the `INDEX.md` row annotation for ADR-279 following the `:339` precedent. Do **not** edit ADR-279's or ADR-282's body (`:359`); cross-reference ADR-282's now-false "still holds" sentence from ADR-285 instead. → Status-column annotation added + ADR-285 row. `git diff --stat` on both bodies: **empty**. `adr-index.test.ts`'s one failure is **ADR-253** (0 `## Consequences` at HEAD) — pre-existing.
- [x] 6.6 Bump the package version (`minor`) and write the `CHANGELOG.md` entry, leading with the breaking default change and naming the two affected templates. → `0.4.16` → **`0.5.0`** (pkg + lockfile); `healthcheck --check=version` PASS.
- [x] 6.7 Update `README.md`, `docs/USERGUIDE.md`, `docs/ARCHITECTURE.md`, `examples/README.md`, and `examples/icm-onboarding/answers.example.json` to drop flag references and state that ICM follows the template. → all five; verified `--help` has no `icm` and no live surface instructs the flag. **Flagged, not fixed:** `USERGUIDE.md` is separately stale from ADR-284 (UI tabs, `.zip`) — recorded in 6.9.
- [x] 6.8 Run the full local gate in the repo's documented order: `npm run build` → `npm --prefix packages/create-agent-harness test` → `node scripts/healthcheck.mjs` → `node scripts/path-guard.mjs` → `node scripts/check-runner-coverage.mjs` → `node examples/vertical-tour/vertical-tour.mjs` → `node scripts/preflight.mjs`. Capture output as proof. → first six **all green** (674 pass / 0 fail; tour `19/19 + 2/2 + 20/20`, `EXIT=0`). `preflight` **6 failures, all attributed**: (1) dirty tree = this unit's own pre-commit work; (2) version drift across 41 packages — **verified also failing at HEAD** (`metaharness=0.4.16`); (3) `evals-extract` README; (4)+(5) `wasm-pack` not installed → size budget cascade (CI installs it, `ci.yml:95-106`); (6) `agntcy` live-server test at `localhost:8888` — package untouched by this diff. `cargo test` **passed** (36/0, 191s) despite the >60s warnings task 1.0 recorded. **None is a spec-02 regression.** Needs Chris's call on whether to accept attributed-red or fix the three repo-side causes.
- [x] 6.9 Record the residual prose surfaces not covered by code: `docs/specs/01-*/01-spec-*.md` and the historical ADR bodies are **left alone** (historical records). Note them so a later grep does not read them as misses. → recorded in `02-task-06-proofs.md` §6.9, incl. the separate `USERGUIDE.md` UI debt.
