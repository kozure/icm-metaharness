...
- **`security.yml` failed on `audit-deps-aggregate` — INHERITED, now REMEDIATED** (run `34782911399`, commit `2bca97d`). First execution ever on this fork. `[audit-deps] FAIL: npm(apps/web-ui) — 2 advisories at high+: @huggingface/transformers(high), sharp(high)`. Verified not fork-caused: `apps/web-ui/package-lock.json`, `apps/web-ui/package.json`, and `scripts/audit-deps.mjs` were each **byte-identical to upstream-at-pin**, and no fork commit had touched a dependency manifest. Upstream's own `security.yml` failed on the **same job**: 3 of its last 6 runs (`34578930431` 09-11, `34455741562` 09-10, `34455706624` 09-10), including failures on days when it also passed.

  Initially left open deliberately: remediating diverges `apps/web-ui` dependencies from upstream, enlarging the re-sync surface on a component the fork does not own. **Owner reversed that call** — a red required gate is itself a defect, and "inherited" is a cause, not a justification. Remediated in `2bca97d`: the `overrides` block had pinned each transitive at the *last vulnerable* version and the ranges already permitted it, so `npm install` could never have moved off them — `sharp "^0.35.3"` → resolved 0.35.3 (vulnerable range `<0.35.4`; libheif advisories GHSA-g89c-p67h-r497 / GHSA-2jg2-4ch7-h545), and `adm-zip "^0.6.0"` → resolved 0.6.0 (top of vulnerable range 0.5.9–0.6.0; symlink-following extraction, GHSA-vwc7-r8mq-g2x9). One patch each — `^0.6.1` / `^0.35.4` — with no dependency added, removed, or repurposed, and **no advisory suppressed**: `scripts/audit-deps.mjs` has no waiver mechanism, so fixing the resolution was the only option that keeps the gate meaningful. Both advisories reach the tree only through `@huggingface/transformers`' *node-only* optionals (`onnxruntime-node` → `adm-zip`; `sharp`); the browser path uses transformers' WASM build, so neither is on the app's runtime path. Lock diff confined to `sharp`, its bundled `@img/sharp-*` platform binaries, and `adm-zip` — nothing else moved.

  Divergence from upstream-at-pin is deliberate and classified as a **security fix, not a merge-safety change**: ADR-279's no-flag byte-identity property is scaffold *output* and is unaffected (that suite passes). Proof the bumped deps work, not merely resolve: `sharp` 0.35.4 loads (libvips 8.18.6) and encodes a PNG; `adm-zip` 0.6.1 round-trips a zip; web-ui 71/71 tests pass including `embeddings.test.ts` (the transformers consumer); `npm run build` succeeds. Locally `[audit-deps] INFO: ALL CLEAN at high+`; in CI all three legs PASS (`npm(workspace)`, `npm(apps/web-ui)`, `cargo`) and `security.yml` completed **green 6/6 jobs**.
...
- Fork failure evidence: `34746383299` (Windows-only `icm-optin.test.ts` path defect; fixed in `9981fbe`), `34746383310` / `34748194360` (`security.yml` → `audit-deps-aggregate`, inherited, remediated in `2bca97d` — see above)
- Fork success evidence: `34782911399` — first **green `security.yml` on `fork/main`** (6/6 jobs, commit `2bca97d`), the run that closed the inherited `audit-deps-aggregate` failure; log shows `[audit-deps] ALL CLEAN at high+` on all three legs
- Upstream `security.yml` flapping on the same job (pre-remediation): `34578930431`, `34455741562`, `34455706624` (failures) vs `34593946538`, `34594118472` (passes, same day)
